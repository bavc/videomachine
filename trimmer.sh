...
ffmpeg -ss ${2} -to ${3} -i "${1}" ${mov_flags} ${color_primaries_flag} ${transfer_characteristics_flag} ${matrix_coefficients_flag} -c copy "${1%.*}_trimmed.${extension}"
printf "\n\n*******START FFMPEG COMMANDS*******\n" >&2
printf "ffmpeg -ss ${2} -to ${3} -i '${1}' ${mov_flags} ${color_primaries_flag} ${transfer_characteristics_flag} ${matrix_coefficients_flag} -c copy '${1%.*}_trimmed.${extension}' \n" >&2
